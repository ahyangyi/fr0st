...
class TestPalette(TestCase):
    ...
    def check_index(self, palette, index, value):
        self.assertEqual(palette[index, 0], value[0], index)
        self.assertEqual(palette[index, 1], value[1], index)
        self.assertEqual(palette[index, 2], value[2], index)
    ...
    def testFromImage(self):
        palette = Palette()
        palette.from_image(list(map(ord, image_data)), (14, 14))
