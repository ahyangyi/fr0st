...
class unAnimatedxForm(Structure):
    _fields_ = [("a", c_float), ("b", c_float), ("d", c_float), ("e", c_float)]
...
def loadFlam4(flame):
    ...
    flam4Flame.size[0] = 100.0 / flame.scale
    flam4Flame.size[1] = (
        -100.0 * (flame.height / float(flame.width)) / flame.scale
    ...
    flam4Flame.background = rgba(
        flame.background[0] / 255.0,
        flame.background[1] / 255.0,
        flame.background[2] / 255.0,
        0,
    )
    ...
    for ci, color in zip(flam4Flame.colorIndex, flame.gradient):
        ci.r, ci.g, ci.b = (i / 255.0 for i in color)
        ci.a = 1
...
def loadXform(inxform, xform):
    ...
    # the color correctly.
    xform.symmetry = 1.0 - inxform.color_speed * 2
    return xform
...
def renderFlam4(flame, size, quality, progress_func, transparent=False, **kwds):
