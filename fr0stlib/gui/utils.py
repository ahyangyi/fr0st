...
def Box(self, name, *a, **k):
    box = wx.StaticBoxSizer(wx.StaticBox(self, -1, name), k.get("orient", wx.VERTICAL))
    box.AddMany(a)
    return box
...
def MakeTCs(parent, *a, **k):
    ...
    for i, default in a:
        ...
        tcs[i] = tc
        fgs.Add(
            wx.StaticText(parent, -1, i.replace("_", " ").title()),
            0,
...
class SizePanel(wx.Panel):
    def __init__(self, parent, callback=lambda: None):
        ...
        fgs, tcs = MakeTCs(
            self, ("width", 512.0), ("height", 384.0), low=0, callback=self.SizeCallback
        )
...
class NumberTextCtrl(wx.TextCtrl):
    @BindEvents
    def __init__(
        self, parent, val=0.0, low=None, high=None, callback=None, int_only=False
    ):
        self.parent = parent
    ...
    def OnChar(self, e):
        ...
        if key in [wx.WXK_RETURN, wx.WXK_NUMPAD_ENTER]:
            ...
            key < wx.WXK_SPACE or key == wx.WXK_DELETE or key > 255 or key == wx.WXK_TAB
        ):
            e.Skip()
...
class MultiSliderMixin(object):
    ...
    def IterSliders(self):
        for name, (_, tc) in list(self.sliders.items()):
            yield name, tc.GetFloat()

    def OnSlider(self, e, tc):
        self._new = True
        tc.SetFloat(e.GetInt() / 100.0)
        e.Skip()
