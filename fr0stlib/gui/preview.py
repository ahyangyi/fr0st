...
class ImageCache(object):
    ...
    def lighten(self):
        ...
        for v, k in sorted((v, k) for (k, v) in list(self.timedict.items())):
...
class PreviewFrame(wx.Frame):
    ...
    def OnResize(self, e):
        ...
        pw, ph = list(map(float, self.GetPanelSize()))
        fw, fh = list(map(float, self.parent.flame.size))
    ...
    def RenderPreview(self, flame=None):
        ...
        pw, ph = list(map(float, self.GetPanelSize()))
        fw, fh = list(map(float, flame.size))
        ratio = min(pw / fw, ph / fh)
        ...
        if bmp is not None:
            self.idlefunc = partial(self.RenderCallback, flamestr, bmp, fromcache=True)
            return

        self.rendering = True
        req = self.parent.renderer.LargePreviewRequest
        req(
            partial(self.RenderCallback, flamestr),
            flame,
            size,
            progress_func=self.prog,
            cancel_func=self.CancelCallback,
...
class PreviewBase(wx.Panel):
    ...
    def Move(self, diff):
        ...
        pixel_per_unit = fw * flame.scale / 100.0
...
class PreviewPanel(PreviewBase):
    ...
    def MoveAndZoom(self):
        ...
        if zoom > 1:
            ...
            newimg.Paste(image, (iw - fw) / 2 - ow / zoom, (ih - fh) / 2 - oh / zoom)
