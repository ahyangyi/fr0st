...
class FavoritesHandler(object):
    def __init__(self, parent):
        ...
        parent.Bind(
            wx.EVT_MENU_RANGE,
            partial(self.OnFavorite, parent),
            id=self.id,
            id2=self.id + 12,
        )
        parent.Bind(wx.EVT_MENU, partial(self.OnManage, parent), id=ID.MANAGE)

        main = parent.parent
        main.menu.GetMenu(3).InsertMenu(2, -1, menu1.name, menu1)
        main.Bind(
            wx.EVT_MENU_RANGE,
            partial(self.OnFavorite, main),
            id=self.id,
            id2=self.id + 12,
        )
        main.Bind(wx.EVT_MENU, partial(self.OnManage, main), id=ID.MANAGE)
    ...
    def Load(self, lst):
        for menu in self.menus:
            ...
            for i, path in enumerate(lst):
                menu.Append(
                    self.id + i, "&%s\tCtrl-F%s" % (os.path.basename(path), i + 1)
                )
        self.max = i + 1
        self.lst = lst
