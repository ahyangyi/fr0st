...
from fr0stlib.gui.utils import LoadIcon, MultiSliderMixin, NumberTextCtrl, MakeTCs
...
class XformTabs(wx.Notebook):
    ...
    def UpdateView(self):
        choices = list(map(repr, self.parent.flame.iter_xforms()))
...
class XformPanel(wx.Panel):
    choices = {
        "rotate": list(map(str, (5, 15, 30, 45, 60, 90, 120, 180))),
        "translate": list(map(str, (1.0, 0.5, 0.25, 0.1, 0.05, 0.025, 0.01))),
        "scale": list(map(str, (1.1, 1.25, 1.5, 1.75, 2.0))),
    }
    ...
    def __init__(self, parent):
        ...
        self.weightszr, d = MakeTCs(self.parent, ("weight", 1), callback=cb, low=0)
        ...
        fgs.AddMany(itertools.chain(*list(zip(xyo_btn, itr, itr))))
        ...
        # Add other buttons
        reset = wx.Button(self, -1, "Reset xform", name="Reset", style=wx.BU_EXACTFIT)
        solo = wx.Button(self, -1, "Make solo", name="Solo", style=wx.BU_EXACTFIT)
        btnszr = wx.BoxSizer(wx.HORIZONTAL)
        btnszr.AddMany((reset, (10, 5), solo))

        # Add the Comboboxes and buttons
        list(map(self.MakeComboBox, *list(zip(*iter(list(self.config.items()))))))

        btn = [
            wx.BitmapButton(self, -1, LoadIcon("xformtab", i), name=i.replace("-", ""))
            for i in (
                "90-Left",
                "Rotate-Left",
                "Rotate-Right",
    ...
    def MakeComboBox(self, name, default):
        cb = wx.ComboBox(
            self, -1, str(default), name=name, size=(80, 28), choices=self.choices[name]
        )
    ...
    def FuncRotateLeft(self, xform):
        xform.rotate(
            self.config["rotate"], pivot=(0, 0) if config["World-Pivot"] else None
        )
    ...
    def UpdateFlame(self, e=None, tempsave=True):
        ...
        if view == "triangle":
            xform.points = list(zip(*[iter(self.coefs)] * 2))
        elif view == "xform":
            xform.coefs = self.coefs
        elif view == "polar":
            xform.polars = list(zip(*[iter(self.coefs)] * 2))
    ...
    @coefs.setter
    def coefs(self, v):
        list(map(lambda x, y: getattr(self, x).SetFloat(y), "adbecf", v))
...
class VarPanel(wx.Panel):
    ...
    def OnKeyDown(self, e):
        ...
        if key in (wx.WXK_NUMPAD_ENTER, wx.WXK_RETURN):
            ...
            child_texts = list(
                filter(
                    lambda x: x.GetClassName() == "wxTextCtrl",
...
class ChaosPanel(wx.Panel):
    ...
    def init_tree(self, label):
        ...
        self.Bind(wx.EVT_TREE_END_LABEL_EDIT, partial(self.OnEndEdit, tree), tree)
