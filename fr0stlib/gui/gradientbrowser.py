...
_ugr_main_re = re.compile(
    "\s*(.*?)\s*{\s*gradient:\s*.*?\s*smooth=(yes|no)\s*(.*?)\}", re.DOTALL
)
...
def _load_ugr_iter(filename):
    ...
    for match in _ugr_main_re.finditer(text):
        ...
        palette = Palette()
        palette[:] = list(zip(newr, newg, newb))
...
def _load_xml_iter(filename):
    ...
    for match in _xml_main_re.finditer(text):
        ...
        palette = Palette()
        palette.data = list(zip(*[(int(i, 16) for i in lst)] * 3))
...
class GradientBrowser(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.bcb = wx.adv.BitmapComboBox(self, style=wx.CB_READONLY)
    ...
    def parse_file(self, path):
        ...
        if ext in (".flame", ".bak"):
            ...
            # three values
            data = [list(map(float, s.split()[0:3])) for s in lns]
            if len(data) != 256:
                raise ParsingError(
