...
class Editmenu(wx.Menu):
    ...
    def __init__(self):
        ...
        self.Append(
            ID.UNDOALL,
            "U&ndo All\tCtrl-Shift-Z",
            " Undo all changes to the current flame.",
        )
        self.Append(ID.REDO, "&Redo\tCtrl-Y", " Redo last change to the current flame.")
        self.Append(
            ID.REDOALL,
            "R&edo All\tCtrl-Shift-Y",
            "Redo all changes to the current flame.",
        )
        self.AppendSeparator()
...
def Create(lst, parent):
    menu = wx.MenuBar()
    list(map(menu.Append, *list(zip(*((menu(), menu.name) for menu in lst)))))
