...
class EditorFrame(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        self.wildcard = "Python source (*.py;*.pyw)|*.py;*.pyw|" "All files (*.*)|*.*"
    ...
    def CheckForChanges(self):
        ...
        if self.tc.GetText() != filetext:
            ...
            dlg = wx.MessageDialog(
                self,
                "Save changes to %s?" % os.path.split(self.scriptpath)[-1],
                "Fr0st",
                wx.YES_NO | wx.CANCEL,
            )
            result = dlg.ShowModal()
...
class MyLog(wx.TextCtrl):
    def __init__(self, parent):
        ...
        wx.TextCtrl.__init__(
            self, parent, -1, style=wx.TE_MULTILINE | wx.TE_READONLY | wx.HSCROLL
        )
...
class CodeEditor(stc.StyledTextCtrl):
    ...
    def SetUpEditor(self):
        ...
        self.MarkerDefine(
            stc.STC_MARKNUM_FOLDERMIDTAIL, stc.STC_MARK_TCORNER, "white", "black"
        )
        self.MarkerDefine(
            stc.STC_MARKNUM_FOLDERTAIL, stc.STC_MARK_LCORNER, "white", "black"
        )
        self.MarkerDefine(
        ...
        self.MarkerDefine(
            stc.STC_MARKNUM_FOLDEROPEN, stc.STC_MARK_BOXMINUS, "white", "black"
        )

        # Global default style
        if wx.Platform == "__WXMSW__":
            self.StyleSetSpec(
                stc.STC_STYLE_DEFAULT, "fore:#000000,back:#FFFFFF,face:Courier New"
            )
        elif wx.Platform == "__WXMAC__":
            # TODO: if this looks fine on Linux too, remove the Mac-specific case
            # and use this whenever OS != MSW.
            self.StyleSetSpec(
                stc.STC_STYLE_DEFAULT, "fore:#000000,back:#FFFFFF,face:Monaco"
            )
        else:
