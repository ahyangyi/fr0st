...
class MainNotebook(wx.Notebook):
    ...
    def UpdateView(self, rezoom=False):
        ...
        self.transform.toolbar.ToggleTool(ID.EditPostXform, config["Edit-Post-Xform"])
    ...
    def AddToolbar(self):
        ...
        def add_tool(name, toggle=False):
            ...
            self.toolbar.AddSimpleTool(
                id, LoadIcon("toolbar", name), name, isToggle=toggle
            )
            if toggle:
...
class GradientPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.slider = wx.Slider(
            self, -1, 0, -180, 180, style=wx.SL_HORIZONTAL | wx.SL_LABELS
        )
        ...
        for k, tcs in list(self.dict.items()):
        ...
        bx = Box(self, "Gradient Generation", opts, btnszr, orient=wx.HORIZONTAL)
    ...
    def MakeTCs(self, *a, **k):
        ...
        fgs.Add((0, 0))
        fgs.AddMany(
            (wx.StaticText(self, -1, i), 0, wx.ALIGN_CENTER) for i in ("Min", "Max")
        )
        for i in a:
            tcs = tuple(NumberTextCtrl(self, **k) for i in range(2))
            self.dict[i] = tcs
            fgs.Add(
                wx.StaticText(self, -1, i.replace("_", " ").title()),
                0,
...
class Gradient(wx.Panel):
    ...
    def OnPaint(self, evt):
        ...
        dc.DrawLines(
            [(i * 1.5, 30 - j * 500) for i, j in enumerate(self.colorhist_array)], 2, 2
        )
...
class AdjustPanel(MultiSliderMixin, wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.bgcolor_panel = wx.Panel(self, size=(64, 10), style=wx.BORDER_SUNKEN)
    ...
    def UpdateFlame(self, tempsave=False):
        ...
        flame.background = tuple(
            c / 255.0 for c in self.bgcolor_panel.GetBackgroundColour()
        )
        self.UpdateView()
        self.parent.image.RenderPreview()
...
class AnimPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        fgs, d = MakeTCs(
            self, ("time", 0), low=0, int_only=True, callback=self.UpdateFlame
        )
        self.dict = d
        ...
        fgs2, d = MakeChoices(
            self,
            *(
                (i, getattr(self, i + "_dict"), None)
                for i in ("interpolation_type", "interpolation", "palette_mode")
            ),
