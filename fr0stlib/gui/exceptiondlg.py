...
class ExceptionDialog(wx.Dialog):
    ...
    def __init__(self, parent, exc_type, exc_value, exc_info, exc_msg):
        wx.Dialog.__init__(self, parent, title="fr0st has encountered an error")
        ...
        scrolled = ScrolledPanel(
            pane, size=(500, 250), style=wx.TAB_TRAVERSAL | wx.SUNKEN_BORDER
        )
        ...
        static = wx.StaticText(
            self, label="fr0st has encountered an error and must exit."
        )
        static.Wrap(500)
        ...
        static = wx.StaticText(
            self,
            label="%s: %s" % (exc_type.__name__, str(exc_value)),
