...
##############################################################################
import os, sys, wx, time, shutil, copy, pickle, itertools, traceback, re
...
class Fr0stApp(wx.App):
    ...
    def SyncUserDirectory(self):
        ...
        for backup_dir in (
            os.path.join(self.user_dir, "backup-%03d") % i for i in itertools.count(1)
        ):
    ...
    def MainLoop(self):
        ...
        if single_instance.IsAnotherRunning():
            ErrorMessage(
                None,
                "Another instance of fr0st is already running. "
    ...
    def Frozen(self):
        return hasattr(sys, "frozen") or hasattr(
            sys, "importers"
        )  # FIXME: there was `imp.is_frozen('__main__')`
    ...
    def LoadIconsInto(self, frame):
        ...
        if "win32" in sys.platform:
            if os.path.exists("icons"):
                icons.AddIcon("icons/fr0st.ico", wx.BITMAP_TYPE_ICO)
            else:
                icons.AddIcon(
                    os.path.join(self.IconsDir, "fr0st.ico"), wx.BITMAP_TYPE_ICO
                )

        if os.path.exists(os.path.join(self.AppBaseDir, "icons")):
            icons.AddIcon(
                os.path.join(self.AppBaseDir, "icons", "fr0st.png"), wx.BITMAP_TYPE_PNG
            )
        else:
            icons.AddIcon(os.path.join(self.IconsDir, "fr0st.png"), wx.BITMAP_TYPE_PNG)

        frame.SetIcons(icons)


class MainWindow(wx.Frame):
    wildcard = "Flame files (*.flame)|*.flame|" "All files (*.*)|*.*"
    scriptrunning = False
...
John Miller"""
    ...
    def OnExit(self, e):
        ...
        if self.renderdialog and self.renderdialog.OnExit() == wx.ID_NO:
            return
        self.OnStopScript()
        if self.editor.IsShown() and self.editor.CheckForChanges() == wx.ID_CANCEL:
            return
    ...
    def RecoverSession(self):
        ...
        config["flamepath"] = path
        recovered = False
        for child, data, (undo, redo) in zip(
            self.tree.GetItemChildren(), self.tree.GetDataGen(), changelist
        ):
            if undo or redo:
                data.extend(undo)
    ...
    def PatchFr0stlib(self):
        ...
        def new_save_flames(path, *flames, **kwds):
            ...
            if kwds:
                raise TypeError("Got unexpected keyword argument: %s" % tuple(kwds)[0])
            if not flames:
            ...
            if os.path.exists(path) and confirm:
                dlg = wx.MessageDialog(
                    self,
                    "%s already exists. Do you want to overwrite?" % path,
                    "Fr0st",
                    wx.YES_NO,
                )
                if dlg.ShowModal() != wx.ID_YES:
            ...
            lst = [s if isinstance(s, str) else s.to_string() for s in flames]
    ...
    def ScriptErrorMessage(self, lines, ty, val, tb):
        ...
        wx.MessageDialog(window, message, "Fr0st", wx.OK | wx.ICON_WARNING).ShowModal()
    ...
    def preview(self, flame=None):
        ...
        time.sleep(0.01)  # Avoids spamming too many requests.
...
class ImagePanel(PreviewBase):
    ...
    def RenderPreview(self, flame=None):
        ...
        ratio = min(230.0 / flame.width, 210.0 / flame.height)
        ...
        req(self.UpdateBitmap, flame.to_string(), size, **config["Preview-Settings"])
