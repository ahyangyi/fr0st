...
def julia_wires():
    # Create new flame
    f = Flame()
    f.gradient.random(hue=(0, 1), saturation=(0, 1), value=(0.25, 1), nodes=(4, 6))
    ...
    if myvar == VAR_JULIAN:
        x = Xform.random(
            f, xv=(VAR_JULIAN, VAR_JULIASCOPE), n=2, xw=0.25, ident=1, col=0
        )
    ...
    xf = f.add_final()
    xf.linear = 0
    xf.spherical = uniform(0.5, 1.5)
    xf.julia = uniform(0.25, 1.25)
    xf.color_speed = 0
    xf.c = uniform(0, 1.5) - 0.75
    xf.f = uniform(0, 1.5) - 0.75
